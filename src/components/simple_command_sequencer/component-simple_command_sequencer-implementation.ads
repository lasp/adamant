...
private
   ...
   -- One frame pool: a contiguous run of frame ids and the round-robin cursor
   -- for the next search in it. An empty pool has Count = 0 and is never
   -- searched.
   type Frame_Pool_Range is record
      First : Simple_Sequencer_Types.Frame_Id_Type := 0;
      Last : Simple_Sequencer_Types.Frame_Id_Type := 0;
      Count : Natural := 0;
      -- One past the frame claimed most recently, so frames are handed out
      -- round robin and the frame released most recently is the last reused.
      Next_Hint : Simple_Sequencer_Types.Frame_Id_Type := 0;
   end record;
   type Frame_Pools is array (Sequence_Enums.Frame_Pool.E) of Frame_Pool_Range;
   ...
   type Instance is new Simple_Command_Sequencer.Base_Instance with record
      ...
      -- The two frame pools, laid out by Init: the waiting pool first, then
      -- the non-waiting pool. Only frame allocation reads them.
      Pools : Frame_Pools;
