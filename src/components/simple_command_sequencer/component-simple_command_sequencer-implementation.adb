...
package body Component.Simple_Command_Sequencer.Implementation is
   ...
   overriding procedure Init (Self : in out Instance; Num_Concurrent_Sequences : in Simple_Sequencer_Types.Num_Concurrent_Sequences_Type; Sequences : in not null Simple_Sequencer_Types.Sequences_Access) is
   begin
      Self.Sequence_Frames := new Simple_Sequencer_Types.Sequence_Frame_Array (0 .. Num_Concurrent_Sequences - 1);
   ...
   begin
      ...
      -- suite from the referencing instance's Num_Concurrent_Sequences init
      -- parameter (see the simple_command_sequencer_packets model), which
      -- makes the per-frame fields visible field-by-field in the ground
      -- system. The FSW does not need that type; it fills the packet buffer
      -- one frame at a time using the Sequence_Frame_Summary serializer.
      declare
