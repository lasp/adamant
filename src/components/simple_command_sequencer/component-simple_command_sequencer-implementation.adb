...
package body Component.Simple_Command_Sequencer.Implementation is
   ...
   -- Wake a parked frame whose wait is over: step past the command or sleep it
   -- was parked on and run it to its next park or to the end of its sequence.
   procedure Wake_And_Execute (Self : in out Instance; Frame : in out Sequence_Frame; Time : in Sys_Time.T) is
   begin
      Frame.Step := @ + 1;
      Frame.Status := Running;
      Execute_Sequence (Self, Frame, Time);
   end Wake_And_Execute;
