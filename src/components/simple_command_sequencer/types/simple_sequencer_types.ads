...
package Simple_Sequencer_Types is
   ...
   -- 0 .. Num_Waiting_Frames - 1, non-waiting sequences on the frames after them.
   -- A non-waiting frame never parks on a response, so a response still in flight
   -- when its run ended has nothing to wake there, and a waiting frame only
   ...
   type Sequencer_Config is record
      ...
      Num_Waiting_Frames : Pool_Size_Type;
      Num_Non_Waiting_Frames : Pool_Size_Type;
   end record;
