...
class sequence_step(object):
    ...
    def validate(self):
        ...
        if self.is_sleep():
            ...
            return
        # A static arg is baked into the step table at build time and a dynamic
        # arg is resolved from the caller's buffer at run time; the generator
        # has no third form yet that combines the two, so any mention of Arg
        # outside a pure reference would be emitted verbatim and fail to
        # compile in the generated package, far from the yaml that caused it.
        if self.arg and re.search(r"\bArg\b", self.arg):
            raise ModelException(
                f"Step {self.index} arg '{self.arg}' mixes a reference to the "
                "sequence argument with other Ada. Mixing literals and sequence "
                "argument fields in one step arg is not yet supported. A step "
                "arg must be either exactly 'Arg' or a field path "
                "'Arg.Field[.Subfield]', or a static Ada expression that does "
                "not mention Arg."
            )
