...
A sequencer instance owns two pools of frames, sized by the suite's \texttt{num\_waiting\_frames} and \texttt{num\_no\_wait\_frames}. A sequence with \texttt{wait\_for\_command\_completion: true} always runs on a frame of the waiting pool (frame ids \texttt{0 .. num\_waiting\_frames - 1}); a sequence that does not wait always runs on a frame of the non-waiting pool (the ids after them). The split exists because sub-command responses are matched to a frame by source id alone and carry no run tag: when a non-waiting run ends, its frame goes idle with responses still in flight to it, and had the frame been handed to a sequence that then parked waiting on the same command id, the stale response would have woken the wrong step. A non-waiting frame never parks on a response, so a late response arriving there has nothing to wake, and a waiting frame only ever receives responses to dispatches that waited. The remaining ways a response can arrive late on a waiting frame -- a step that timed out, or a frame killed while parked -- are fault paths, guarded as in the full command sequencer by the pending-command-id match. Within each pool frames are handed out round robin, so the frame released most recently is the last one of its pool reused. A pool may be empty when no sequence of its kind exists; the model rejects a suite whose sequences need a frame from an empty pool. Because the pools are static, idle frames of one pool cannot serve the other: \texttt{No\_Frame\_Available} reports the pool that was full.
