...
In addition to the sequence tables above, this same model drives the autocoded synthesis of one first-class command per declared sequence on the Simple Command Sequencer instance. Discovery is init-parameter-driven: at assembly build time the sequencer's commands model reads each instance's \texttt{Config} init parameter, derives the suite package named there (e.g.\ \texttt{Test\_Assembly\_Command\_Sequences\_Example\_Sequences.Config}), and loads exactly that suite's model, injecting one command per declared sequence into the instance's command set. A naked component build (no assembly, hence no instance and no \texttt{Config} value) produces only the four static commands (\texttt{Run\_Sequence}, \texttt{Kill\_All\_Sequences}, \texttt{Kill\_Frame}, \texttt{Set\_Summary\_Packet\_Period}); a suite merely present on the build path contributes nothing -- only the suite an instance names in its \texttt{Config} adds commands, so two sequencer instances in one assembly can carry different suites. Each per-sequence command (e.g.\ \texttt{Sequence\_B}) takes the sequence's own \texttt{arg\_type} verbatim as its command argument (or no argument for an argless sequence) -- a user-written, normally-registered type, so no argument records are generated or registered anywhere in the pipeline. At run time the component copies the received argument buffer unchanged into a generic \texttt{Run\_Sequence\_Arg.T} and dispatches through the existing \texttt{Run\_Sequence} backbone using the sequence's runtime slot index. \texttt{Run\_Sequence} rejects any argument whose length differs from the serialized length of the sequence's \texttt{arg\_type} (recorded in the autocoded table) with an \texttt{Invalid\_Sequence\_Argument\_Length} event; the response behavior comes from the sequence's static configuration in the same table (see Sequence Response Behavior below). A companion generated package, \texttt{<suite>\_Commands}, carries the operator-side builder surface (per-sequence command id getters and \texttt{Command.T} constructors) for unit tests and other on-board callers -- split from the sequences package because the sequencer component itself never uses the builders. No per-sequence Ada needs to be hand-written:
...
Because every sequence is a first-class command on the sequencer instance, a sequence step may invoke another sequence simply by targeting the sequencer's own synthesized command, e.g.\ \texttt{command: My\_Sequencer.Sequence\_C}. No special syntax or generator support is involved -- the step resolves like any other \texttt{Component.Command} reference, provided the sequencer instance is declared in the assembly. At run time the sub-sequence command is emitted out the sequencer's \texttt{Command\_T\_Send} connector, routed through the command router, and arrives back at the sequencer's \texttt{Command\_T\_Recv\_Async}, where it claims its own frame and executes concurrently with (and independently of) the calling sequence. If the calling sequence waits for command completion, the sub-sequence's command response (per its static \texttt{response\_behavior} configuration, see below) is what wakes it. Nesting depth is naturally capped by the size of the frame pool the sub-sequence draws from (see Frame Pools below): a sub-sequence call when that pool has no idle frame is rejected with a \texttt{No\_Frame\_Available} event naming the sequence and the pool, and a Failure response to the calling frame. One shape is refused at generation time: a cycle of calls in which every sequence has \texttt{response\_behavior: send\_after\_sequence\_completion} and waits for command completion (a deferred sequence calling itself included). A waiting call into a deferred sequence parks the caller's frame until the callee finishes, so around such a cycle no run could ever finish and frames would be consumed until dispatch fails. A cycle that passes through a \texttt{send\_after\_sequence\_start} sequence, or through a sequence that does not wait for command completion, completes one run per lap and is allowed, so a sequence may deliberately loop through itself either way.

\section{Frame Pools}

A sequencer instance owns two pools of frames, sized by the suite's \texttt{num\_waiting\_for\_response\_frames} and \texttt{num\_non\_waiting\_for\_response\_frames}. A sequence with \texttt{wait\_for\_command\_completion: true} always runs on a frame of the waiting-for-response pool (frame ids \texttt{0 .. num\_waiting\_for\_response\_frames - 1}); a sequence that does not wait always runs on a frame of the non-waiting-for-response pool (the ids after them). The split exists because sub-command responses are matched to a frame by source id alone and carry no run tag: when a non-waiting-for-response run ends, its frame goes idle with responses still in flight to it, and had the frame been handed to a sequence that then parked waiting on the same command id, the stale response would have woken the wrong step. A non-waiting-for-response frame never parks on a response, so a late response arriving there has nothing to wake, and a waiting-for-response frame only ever receives responses to dispatches that waited. The remaining ways a response can arrive late on a waiting frame -- a step that timed out, or a frame killed while parked -- are fault paths, guarded as in the full command sequencer by the pending-command-id match. Within each pool frames are handed out round robin, so the frame released most recently is the last one of its pool reused. Each pool is sized exactly to its users: the model rejects a suite whose sequences need a frame from an empty pool, and equally a suite that gives frames to a pool no sequence draws from, since nothing could ever claim them. Both keys are optional and default to zero, so a suite whose sequences all wait declares only \texttt{num\_waiting\_for\_response\_frames}. Because the pools are static, idle frames of one pool cannot serve the other: \texttt{No\_Frame\_Available} reports the pool that was full.

\section{Sequence Response Behavior}

When the sequencer replies to the command that started a sequence is controlled by the \texttt{Sequence\_Response\_Behavior} enumeration. It is static per-sequence configuration: the optional \texttt{response\_behavior:} field on each sequence in the YAML, baked into the autocoded sequence table and used by both the synthesized per-sequence commands and the generic \texttt{Run\_Sequence} command:
