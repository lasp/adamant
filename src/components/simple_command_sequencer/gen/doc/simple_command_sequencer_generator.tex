...
The following is an example command sequences input YAML file. It defines thirteen sequences exercising the main features: statically-armed commands and static sleeps, typed argument records whose fields are dispatched as dynamic arguments to sub-commands (including through non-byte-aligned packed fields and validatable records, and mixed with literals in one aggregate), a dynamic sleep whose duration comes from the sequence argument, no-wait direct dispatch (sequence-wide and per-step via \texttt{wait\_for\_completion}), a sequence that calls another sequence as a sub-sequence step, and sequences whose command response is statically deferred until completion. This example adheres to the schema shown in the previous section, and is commented to give clarification.
...
A sequence step may forward a field of the parent sequence's argument to the sub-command it dispatches, using the \texttt{Arg.field.subfield} syntax in the step's \texttt{arg:}, or embed such references in a larger Ada expression, e.g.\ \texttt{(Mode => Arg.Mode, Enable => True)}, to mix per-call values with literals in one sub-command argument. Each such step produces a typed Resolver in the generated package: at run time the sequencer dispatches that Resolver, which first validates the per-call argument buffer through the input type's generated \texttt{Validation.Valid} (a failure aborts the sequence with an \texttt{Invalid\_Dynamic\_Command\_Argument} event), then deserializes it through the input type's safe \texttt{Serialization.From\_Byte\_Array}, evaluates the step's expression with \texttt{Arg} bound to the result, and serializes that as the sub-command's argument buffer. The resolver's signature accepts only a \texttt{Basic\_Types.Byte\_Array} -- there is no \texttt{'Address} reinterpretation -- so the dynamic-argument machinery is type-safe by construction; a resolver cannot overlay an arbitrary record onto raw memory, only deserialize a byte array through the input type's generated deserializer.
