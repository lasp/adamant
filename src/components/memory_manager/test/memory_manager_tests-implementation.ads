...
private
   ...
   -- This unit test exercises all possible combinations of initializing the component to make sure only valid initializations succeed, and others throw an assertion.
   overriding procedure Test_Init (Self : in out Instance);
   -- This unit test exercises the nominal request/release of the component's memory region.
   overriding procedure Test_Nominal_Request_Release (Self : in out Instance);
   -- This unit test exercises the error conditions that can arise when requesting/releasing the component's memory region.
   overriding procedure Test_Off_Nominal_Request_Release (Self : in out Instance);
   -- This unit test exercises the memory dump commands.
   overriding procedure Test_Nominal_Memory_Dump (Self : in out Instance);
   -- This unit test exercises the memory crc commands.
   overriding procedure Test_Nominal_Memory_Crc (Self : in out Instance);
   -- This unit test exercises the memory write command.
   overriding procedure Test_Nominal_Memory_Write (Self : in out Instance);
   -- This unit test exercises the write command when the memory is not available.
   overriding procedure Test_Write_Unreleased_Region (Self : in out Instance);
   -- This unit test exercises the memory dump command with invalid memory regions.
   overriding procedure Test_Dump_Invalid_Region (Self : in out Instance);
   -- This unit test exercises the crc command with invalid memory regions.
   overriding procedure Test_Crc_Invalid_Region (Self : in out Instance);
   -- This unit test exercises the write command with invalid memory regions.
   overriding procedure Test_Write_Invalid_Region (Self : in out Instance);
   -- This unit test exercises the force release command.
   overriding procedure Test_Force_Release_Command (Self : in out Instance);
   -- This unit test exercises the behavior when the internal queue overflows.
