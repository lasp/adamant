...
package body Zero_Divider_Cpp_Tests.Implementation is
   ...
   exception
      when E : others =>
         ...
         -- Verify the unexpected result event did not fire:
         Natural_Assert.Eq (T.Unexpected_Int_Divide_By_Zero_Result_History.Get_Count, 0);
   ...
   exception
      when E : others =>
         ...
         -- Verify the unexpected result event did not fire:
         Natural_Assert.Eq (T.Unexpected_Fp_Divide_By_Zero_Result_History.Get_Count, 0);
         -- Verify only 1 event total was sent:
         Natural_Assert.Eq (T.Event_T_Recv_Sync_History.Get_Count, 1);
   ...
   exception
      when E : others =>
         ...
         -- Verify the unexpected result event did not fire:
         Natural_Assert.Eq (T.Unexpected_Raising_Exception_In_Cpp_Result_History.Get_Count, 0);
         -- Verify only 1 event total was sent:
         Natural_Assert.Eq (T.Event_T_Recv_Sync_History.Get_Count, 1);
