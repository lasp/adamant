...
package body Component.Zero_Divider_Cpp.Implementation is
   ...
   begin
      ...
      if Zerodividercpp_Checkmagicnumber (Self.Zero_Divider_Cpp, Arg.Value) = False then
         ...
         declare
            Result : constant Interfaces.Unsigned_32 := Zerodividercpp_Dividebyzero (Self.Zero_Divider_Cpp);
         begin
            -- This will never execute.
            Self.Event_T_Send_If_Connected (Self.Events.Dividing_By_Zero_In_Cpp (Self.Sys_Time_T_Get, (Value => Natural (Result))));
            -- Note to Adamant developers: To be clear, the normal operation of this command will cause an unhandled exception. This will result in a memory leak, as the destructor for the cpp object is never called. As this component is intended for testing the Last Chance Handler (LCH), and recovery from LCH without a full reset is not supported, this memory leak is acceptable.
         end;
      end if;
