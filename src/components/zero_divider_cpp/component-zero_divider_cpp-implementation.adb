...
package body Component.Zero_Divider_Cpp.Implementation is
   ...
   begin
      ...
      Self.Zero_Divider_Cpp := Zerodividercpp_Createwithmagicnumber (Magic_Number);
   end Init;
   ...
   begin
      ...
      if Zerodividercpp_Checkmagicnumber (Self.Zero_Divider_Cpp, Arg.Value) = False then
         ...
         begin
            Result := Zerodividercpp_Dividebyzero (Self.Zero_Divider_Cpp);
            -- This will never execute.
            Self.Event_T_Send_If_Connected (Self.Events.Dividing_By_Zero_In_Cpp (Self.Sys_Time_T_Get, (Value => Natural (Result))));
            -- Note to Adamant developers: To be clear, the normal operation of this command will cause an unhandled exception. This will result in a memory leak, as the destructor is never called. As this component is intended for testing the Last Chance Handler (LCH), and recovery from LCH without a full reset is not a paradigm respected by Adamant, this memory leak is acceptable.
            Zerodividercpp_Destroy (Self.Zero_Divider_Cpp);
         end;
      end if;
