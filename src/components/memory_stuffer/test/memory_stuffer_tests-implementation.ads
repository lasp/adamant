...
private
   ...
   -- This unit test exercises stuffing a region that is unprotected.
   overriding procedure Test_Unprotected_Stuffing (Self : in out Instance);
   -- This unit test exercises stuffing a region that is protected.
   overriding procedure Test_Protected_Stuffing (Self : in out Instance);
   -- This unit test exercises all of the ways that the arm command can be invalidated prior to a write (except via timeout).
   overriding procedure Test_Arm_Unarm (Self : in out Instance);
   -- This unit test exercises the unarming of the arm command via timeout.
   overriding procedure Test_Arm_Timeout (Self : in out Instance);
   -- This unit test exercises writing to an invalid region of memory
   ...
   -- This unit test exercises the memory region copy and release connectors.
