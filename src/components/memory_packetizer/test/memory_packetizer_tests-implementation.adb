...
package body Memory_Packetizer_Tests.Implementation is
   ...
   begin
      ...
      Self.Tester.Component_Instance.Init (Max_Packets_Per_Time_Period => 3, Time_Period_In_Seconds => 5, Max_Packet_Ids => 2);
   end Set_Up_Test;
   ...
   begin
      ...
      for Idx in 1 .. 4 loop
         Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Id, 7);
      end loop;
      for Idx in 5 .. 8 loop
         Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Id, 28);
      end loop;
      ...
      for Idx in 5 .. 8 loop
         Sequence_Count_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Sequence_Count, Sequence_Count_Mod_Type (Idx - 5));
      end loop;
      ...
      for Idx in 13 .. 16 loop
         Sequence_Count_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Sequence_Count, Sequence_Count_Mod_Type (Idx - 9));
      end loop;
   ...
   begin
      ...
      -- Check the packet ids:
      for Idx in 1 .. 4 loop
         Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Id, 4);
      end loop;
   ...
   overriding procedure Test_Max_Packet_Id_Exceeded (Self : in out Instance) is
      use Byte_Array_Pointer;
      use Packet_Types;
      T : Component.Memory_Packetizer.Implementation.Tester.Instance_Access renames Self.Tester;
      A_Packet : Packet.T;
      ...
      Dump_3 : constant Memory_Packetizer_Types.Memory_Dump := (Id => 3, Memory_Pointer => From_Address (Bytes'Address, Bytes'Length));
      Dump_4 : constant Memory_Packetizer_Types.Memory_Dump := (Id => 4, Memory_Pointer => From_Address (Bytes'Address, Bytes'Length));
      Bytes_2 : aliased Basic_Types.Byte_Array := [0 .. 2 * Packet_Data_Length - 1 => 8];
      Dump_5 : constant Memory_Packetizer_Types.Memory_Dump := (Id => 1, Memory_Pointer => From_Address (Bytes_2'Address, Bytes_2'Length));
      Dump_6 : constant Memory_Packetizer_Types.Memory_Dump := (Id => 2, Memory_Pointer => From_Address (Bytes_2'Address, Bytes_2'Length));
      Dump_7 : constant Memory_Packetizer_Types.Memory_Dump := (Id => 3, Memory_Pointer => From_Address (Bytes_2'Address, Bytes_2'Length));
      Dump_8 : constant Memory_Packetizer_Types.Memory_Dump := (Id => 4, Memory_Pointer => From_Address (Bytes_2'Address, Bytes_2'Length));
   begin
      -- Send memory dumps to fill up queue:
      T.Memory_Dump_Send (Dump_1);
      T.Memory_Dump_Send (Dump_2);
      ...
      -- Make sure some events were thrown:
      Natural_Assert.Eq (T.Event_T_Recv_Sync_History.Get_Count, 2);
      Natural_Assert.Eq (T.Max_Packet_Id_Exceeded_History.Get_Count, 2);
      Packet_Id_Assert.Eq (T.Max_Packet_Id_Exceeded_History.Get (1).Id, 3);
      Packet_Id_Assert.Eq (T.Max_Packet_Id_Exceeded_History.Get (2).Id, 4);

      -- Make sure packets were output regardless for each.
      Natural_Assert.Eq (T.Packet_T_Recv_Sync_History.Get_Count, 4);

      -- Check the packet ids:
      for Idx in 1 .. 4 loop
         Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Id, Packet_Id (Idx));
      end loop;

      -- Check packet sequence counts, all should be zero:
      for Idx in 1 .. 4 loop
         Sequence_Count_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Sequence_Count, 0);
      end loop;

      -- Check packet lengths, all should be 15:
      for Idx in 1 .. 4 loop
         Natural_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (Idx).Header.Buffer_Length, 16 + Memory_Region.Serialization.Serialized_Length);
      end loop;
      ...
      T.Memory_Dump_Send (Dump_5);
      T.Memory_Dump_Send (Dump_6);
      T.Memory_Dump_Send (Dump_7);
      T.Memory_Dump_Send (Dump_8);
      ...
      Natural_Assert.Eq (T.Max_Packet_Id_Exceeded_History.Get_Count, 4);
      Packet_Id_Assert.Eq (T.Max_Packet_Id_Exceeded_History.Get (3).Id, 3);
      Packet_Id_Assert.Eq (T.Max_Packet_Id_Exceeded_History.Get (4).Id, 4);

      -- Make sure packets were output regardless for each.
      Natural_Assert.Eq (T.Packet_T_Recv_Sync_History.Get_Count, 12);

      -- Check the packet ids:
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (5).Header.Id, Packet_Id (1));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (6).Header.Id, Packet_Id (1));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (7).Header.Id, Packet_Id (2));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (8).Header.Id, Packet_Id (2));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (9).Header.Id, Packet_Id (3));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (10).Header.Id, Packet_Id (3));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (11).Header.Id, Packet_Id (4));
      Packet_Id_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (12).Header.Id, Packet_Id (4));

      -- Check packet sequence counts:
      Sequence_Count_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (5).Header.Sequence_Count, 1);
      ...
      -- Check packet contents:
      Byte_Array_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (5).Buffer (Mem_Region_Length .. T.Packet_T_Recv_Sync_History.Get (5).Header.Buffer_Length - 1), [0 .. Packet_Data_Length - 1 => 8]);
      Byte_Array_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (6).Buffer (Mem_Region_Length .. T.Packet_T_Recv_Sync_History.Get (6).Header.Buffer_Length - 1), [0 .. Packet_Data_Length - 1 => 8]);
      Byte_Array_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (7).Buffer (Mem_Region_Length .. T.Packet_T_Recv_Sync_History.Get (7).Header.Buffer_Length - 1), [0 .. Packet_Data_Length - 1 => 8]);
      Byte_Array_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (8).Buffer (Mem_Region_Length .. T.Packet_T_Recv_Sync_History.Get (8).Header.Buffer_Length - 1), [0 .. Packet_Data_Length - 1 => 8]);
      Byte_Array_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (9).Buffer (Mem_Region_Length .. T.Packet_T_Recv_Sync_History.Get (9).Header.Buffer_Length - 1), [0 .. Packet_Data_Length - 1 => 8]);
      Byte_Array_Assert.Eq (T.Packet_T_Recv_Sync_History.Get (10).Buffer (Mem_Region_Length .. T.Packet_T_Recv_Sync_History.Get (10).Header.Buffer_Length - 1), [0 .. Packet_Data_Length - 1 => 8]);
