...
package body Parameters_Tests.Implementation is
   ...
   begin
      ...
      Parameter_Id_Assert.Eq (T.Parameter_Update_Success_History.Get (1), (Id => 3));
      ...
      Parameter_Id_Assert.Eq (T.Parameter_Update_Success_History.Get (2), (Id => 2));
   ...
   begin
      ...
      Parameter_Id_Assert.Eq (T.Parameter_Update_Id_Not_Recognized_History.Get (1), (Id => 99));

      -- Send a command to update a parameter value with bad length.
      Param.Header.Id := 3;
      Param.Header.Buffer_Length := 30;
      pragma Assert (T.Commands.Update_Parameter (Param, Cmd) = Success);
      T.Command_T_Send (Cmd);
      Natural_Assert.Eq (T.Dispatch_All, 1);
      Natural_Assert.Eq (T.Command_Response_T_Recv_Sync_History.Get_Count, 2);
      ...
      Invalid_Parameter_Length_Assert.Eq (T.Parameter_Update_Length_Mismatch_History.Get (1), (Header => Param.Header, Expected_Length => 2));
