...
class assembly_cpu_monitor_xml(assembly_hydra_generator, generator_base):
    """Hydra file"""
    def __init__(self):
        this_file_dir = os.path.dirname(os.path.realpath(__file__))
        template_dir = os.path.join(this_file_dir, ".." + os.sep + "templates")
        assembly_hydra_generator.__init__(
            self, "name_cpu_monitor.xml", template_dir=template_dir, subdir="Pages"
        )


# Add all the basic generators for normal packets, except use the cpu_monitor_packets model.
# This allows us to build all the normal packet outputs like html, tex, etc.
add_basic_generators_to_module(
    cpu_monitor_packets.cpu_monitor_packets, packet_templates, module=globals()
)
