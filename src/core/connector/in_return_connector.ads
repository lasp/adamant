...
package In_Return_Connector is
   -- Make specified type visible
   subtype T is Connector_Type;
   subtype R is Return_Type;
   ...
   type Invokee_Hook is access function (A_Component : in out Component.Core_Instance'Class; Input : in T; Index : in Connector_Index_Type) return R;
   ...
   -- to the connector:
   function Call (Self : in Instance; Input : in T) return R
      with Inline => True;
