...
\begin{spacedenumerate}
  ...
  \item \textbf{Component Base Initialization} - A \texttt{Init\_Base} procedure is autocoded for any component that requires heap allocation for base package objects (queues and unconstrained arrayed connectors) or that declares parameters, whose compiled-in default values \texttt{Init\_Base} asserts are valid.
...
\begin{spaceditemize}
  ...
  \item The component has a parameters model, in which case \texttt{Init\_Base} asserts that the component's compiled-in default parameter values pass its \texttt{Validate\_Parameters} function.
\end{spaceditemize}
\vspace{5mm} %5mm vertical space

The first two conditions require variable sized allocations to be made on the heap, provided via parameters to the component base package \texttt{Init\_Base} procedure; the last condition adds no parameters, so a component that only declares parameters receives a parameterless \texttt{Init\_Base}. Below is an example component model that satisfies both of the first two conditions above: \\
...
\vspace{5mm} %5mm vertical space
\begin{spacedenumerate}
  \item \textbf{Init\_Base} - This procedure calls \texttt{Init\_Base} for every component that contains an \texttt{Init\_Base} procedure. This allocates all heap space for arrayed connectors and internal queues, and asserts that the compiled-in default parameter values of any component with parameters are valid. See Section \ref{Component Base Initialization} for more details.
