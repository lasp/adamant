...
You can now modify \textit{project.configuration.yaml} and version control it within your project directory. To tell Adamant to use this file instead of the default configuration file, you need to set the \texttt{ADAMANT\_CONFIGURATION\_YAML} variable, i.e.
...
It is advisable to include the export command above within your environment files that get sourced whenever a new shell is opened, i.e. from .bashrc. \\
...
\begin{spaceditemize}
  ...
  \item \textbf{\texttt{build/template}} - autogenerated source files that are meant to be copied out and used as starting points (i.e. ``templates") for handwritten code
...
\begin{spaceditemize}
  ...
  \item \textbf{\texttt{redo test\_all}} - finds and runs all unit tests from this directory and all below it. Note: Add a \textit{.skip\_test} file to a directory to exclude it from being run by \texttt{redo test\_all}.
  \item \textbf{\texttt{redo coverage\_all}} - finds and runs all unit tests from this directory and all below it and generates coverage reports for each. Note: Add a \textit{.skip\_test} or \textit{.skip\_coverage} file to a directory to exclude it from being run by \texttt{redo coverage\_all}.
  \item \textbf{\texttt{redo publish}} - builds all autocoded documentation from this directory and all below it, and copies over the ``published" version of that documentation
  ...
  \item \textbf{\texttt{redo analyze\_all}} - finds and runs static analysis on all unit tests from this directory and all below it. Note: Add a \textit{.skip\_test} file to a directory to exclude it from being run by \texttt{redo analyze\_all}.
\end{spaceditemize}
\vspace{5mm} %5mm vertical space
...
\vspace{5mm} %5mm vertical space
\begin{spacedenumerate}
  \item The set of code that is used to compile for the target, i.e. \textit{the build path} discussed in the next section
  \item The platform for which the code is being compiled, i.e. the name of the compiler, linker, and binding programs
...
The Ada compiler used in Adamant is generally GNAT based \texttt{gcc} for Native Linux compiles, and \texttt{gcc}-based cross compilers for embedded targets (i.e. \texttt{arm-eabi-gcc} for ARM). The compiler, compilation flags, and runtime are all determined by the \textit{build target}. The Adamant \textit{build path} determines which source code is compilable for a given \textit{build target} and is discussed in the following section. To add a custom build target for your system see Section \ref{Adding a Build Target} for details. \\
...
The target can also be set globally in your shell configuration file (i.e. \textit{.bashrc}) or from any local \texttt{.do} or \texttt{env.py} file. To see the all the supported build targets on your system run:
...
\begin{spacedenumerate}
  ...
  \item Any directory found with a \texttt{.TARGET\_path} file present, where \texttt{TARGET} is the name of the currently set build target (i.e. the \texttt{TARGET} environment variable), is added to the build path.
...
Adamant aims to solve these problems through the use of \textit{packed types}.  In Adamant, packed types are modeled using a YAML specification. From the specification, the following type definitions are generated:
...
Using the serialization and deserialization functions shown in the previous section can be error prone. Consider the field in our record \texttt{Value\_2}, which only has a valid integer range between -2 and 3. It is possible that when calling the deserialization function \texttt{From\_Byte\_Array}, the incoming byte array might contain a bit representation that puts the \texttt{Value\_2} field out of range, i.e. 5. The call to \texttt{From\_Byte\_Array} will not fail, but any subsequent operation using the \texttt{packed\_Type} variable will fail with an Ada constraint error, due to the range check failing on \texttt{Value\_2}. \\
...
Note that a \texttt{Valid} function which returns a \texttt{Boolean} is available to validate the serialized byte representation of a packed record. The function accepts a \texttt{Serialization.Byte\_Array} and checks that all fields are within their valid ranges before the data is deserialized into a typed record. The Adamant component autocoder uses this feature extensively to validate incoming commands and parameters prior to their use within user code.\\
...
\begin{spaceditemize}
  ...
  \item \textbf{\texttt{\_\_eq\_\_}} - Compares two packed records for equality. For packed records containing floating-point fields, an optional \texttt{epsilon} parameter can be passed to allow tolerant comparisons (e.g., \texttt{a.\_\_eq\_\_(b, epsilon=0.0001)}). See below for additional details on epsilon comparisons.
...
\textbf{Epsilon Comparisons for Floating-Point Fields} \\

When packed records contain floating-point fields, exact equality comparisons may fail due to floating-point representation differences. Adamant provides an \texttt{epsilon} context manager and related utilities to enable tolerant comparisons. These can be imported from the base classes module:
...
\vspace{5mm} %5mm vertical space
\begin{spaceditemize}
  \item \textbf{\texttt{epsilon(value)}} - A context manager that temporarily sets the tolerance for float comparisons within its scope. When comparing packed records inside a \texttt{with epsilon(...):} block, floating-point fields are considered equal if they differ by less than the specified epsilon value. After the context exits, the epsilon reverts to its previous value.
  \item \textbf{\texttt{set\_default\_epsilon(value)}} - Sets a persistent default epsilon value for the current context. This is useful in environments where tolerant comparisons are always desired without repeatedly using the context manager.
  \item \textbf{\texttt{get\_epsilon()}} - Returns the current epsilon value (default is \texttt{0.0}, meaning exact comparison).
\end{spaceditemize}
\vspace{5mm} %5mm vertical space

The epsilon tolerance propagates to nested packed records and arrays, so comparisons of complex nested structures will use the same tolerance for all floating-point fields. An example of how these utilities can be used is shown below:

\vspace{5mm} %5mm vertical space
\begin{minted}{python}
from base_classes.packed_type_base import epsilon

# Exact comparison (default behavior, epsilon=0.0)
record1 = My_Record(Value=1.0000001)
record2 = My_Record(Value=1.0000002)
assert record1 != record2  # Different with exact comparison

# Tolerant comparison using context manager
with epsilon(0.0001):
    assert record1 == record2  # Equal within tolerance

# Back to exact comparison after context exits
assert record1 != record2

# Explicit epsilon parameter on __eq__
assert record1.__eq__(record2, epsilon=0.0001)  # Equal within tolerance
\end{minted}
\vspace{5mm} %5mm vertical space

A more complete example of how the python class can be used is shown below in a file called \textit{test.py}:
...
\vspace{5mm} %5mm vertical space
\begin{spaceditemize}
  \item \textbf{\texttt{Example\_Record}} - The constructor used to create a packed record object. The record field values are optionally required upon initialization. The class performs extensive type checking of the record fields to make sure that their values adhere to ranges of the equivalent Ada types, where possible.
...
Packed records or packed arrays (discussed in Section \ref{Packed Arrays}) can also contain fields that are themselves packed records (or arrays). These types can be nested arbitrarily deep, but there are some limitations on nesting that will be discussed in this section. \\

See an example below, a packed record that includes two fields that are of packed record type:

\yamlcodef{../example_architecture/nested_record/nested_record.record.yaml}
...
\vspace{5mm} %5mm vertical space
\begin{spacedenumerate}
  \item There may only be a single variable length field, i.e. \textit{array of values}, in a packed record.
...
\adacodef{../example_architecture/build/src/example_record-c.ads}
...
Each field of \texttt{U\_C} uses the C version of its type, so the record lays out like a C struct declared with the same members. A nested packed type appears as its own \texttt{U\_C}. A modeled enumeration appears as its C version, \texttt{E\_C}, which has the size of a C \texttt{int} (see Section \ref{C Enumerations}). A \texttt{Boolean} appears as \texttt{Interfaces.C.C\_bool}, the Ada standard type for a C \texttt{bool}. The conversion functions convert each of these fields along with the rest of the record, so a C struct with an \texttt{enum} or \texttt{bool} member can be modeled directly with the enumeration or \texttt{Boolean} type in the YAML.

\subsection{Packed Arrays} \label{Packed Arrays}
...
Packed arrays with elements of type packed arrays or packed records are allowed. This is specified similarly to the way it is done for packed records. See Section \ref{Nested Packed Records} for details.

\subsubsection{C/C++ Packed Arrays}

Packed arrays can be converted to a C/C++ type in a similar way to packed records. See Section \ref{C Packed Records} for details. An element of \texttt{U\_C} uses the C version of its type in the same way as a record field: a packed type as its own \texttt{U\_C}, a modeled enumeration as \texttt{E\_C}, and a \texttt{Boolean} as \texttt{Interfaces.C.C\_bool}.
...
\subsubsection{C/C++ Enumerations} \label{C Enumerations}

Each enumeration package also contains a nested package named \texttt{C} which holds a C version of \texttt{E} suitable for passing to a binded C or C++ function. See Section \ref{C Interfacing}. This is needed because C and Ada use different sizes to hold an enumeration. C stores an \texttt{enum} as an \texttt{int}, while Ada stores \texttt{E} in the smallest size that fits its literals, so passing \texttt{E} to C directly would be an error. The C version, \texttt{E\_C}, is declared with \texttt{Convention => C}, which gives it the size of a C \texttt{int}, and has the same literal names and values as \texttt{E}. \\

Two functions are provided: \texttt{To\_C} to convert an \texttt{E} to an \texttt{E\_C} before calling into C or C++, and \texttt{To\_Ada} to convert an \texttt{E\_C} returned from C or C++ back to an \texttt{E}. Both map by literal value, so \texttt{To\_Ada} raises \texttt{Constraint\_Error} when given an integer that is not a literal of the enumeration. The \texttt{'Valid} attribute can be used on an \texttt{E\_C} to check for this first. Using these functions encapsulates the C type to the binding itself, so the rest of the Ada code can use \texttt{E} and everything Adamant generates for it.
...
These pragmas can be applied to types, i.e.
...
The first thing to recognize is the type \texttt{Instance} which extends the type \texttt{Component.Instance}, declared in the framework's core \texttt{Component} package. The details of the \texttt{Instance} type are private, and thus listed in the private section. Also, note that the type is declared as \texttt{abstract} since it contains some abstract subprograms, i.e. primitives that are unimplemented. This means that \texttt{Instance} cannot be instantiated or used by itself, since it is not a complete type. We must inherit from this type and implement the abstract subprograms before we can instantiate an object that uses this code. We will see how that is done in the next section. \\
...
We can map the files above to their package names in the package diagram shown in Section \ref{Anatomy of a Component Unit Test}. Below is the map of package names to the location of their autogenerated source:
...
In this case, we have added the line \texttt{Self.Tester.Expect\_Packed\_U16\_T\_Send\_Dropped := True;} which tells the tester component that the next connector call will overflow the queue. When this is set, the component will not throw an assertion. See the new output below.
...
As discussed in the previous section, active components in Adamant almost always have an asynchronous queue which aids in metering the execution of that component. There are a few instances where not having an asynchronous connector makes sense, mostly in the case of the lowest priority background component (i.e. memory scrubbing), or a component whose execution is metered internally by sleeping periodically. This section will briefly show a component of this nature and explain how to unit test it.
...
For each data dependency there are two functions provided. The first retrieves the data dependency's ID, i.e. \texttt{Get\_Counter\_Id}, and the other parses out the data dependency type from a generic data product, i.e. \texttt{Extract\_Counter}. Again, these functions rarely need to be called directly by component implementation code, but it is good to know they exist. We will discuss how they get used in the following section.
...
\begin{spacedenumerate}
  ...
  \item If all the previous checks pass, the parameter is ``staged", i.e. stored in a \textit{staged} variable.
...
Note that after validation, a parameters is not immediately copied over to the component \texttt{Instance} variables holding the parameters. Instead the parameters are ``staged", i.e. copied to a temporary staging variable. This is done for a few reasons:
...
By default, PDF files are generated in the \textit{build/pdf/} directory. This allows the developer to review the output documentation before ``publishing" it, i.e. saving it in the repository. Once the documentation looks good, you can publish it manually by copying it into the \textit{doc/} directory:
...
This section shows the IDed entity models for the example components presented in Section \ref{The Example Components}. If you are unfamiliar with these model types see Section \ref{Component IDed Entities}. \\
...
Sometimes you want Adamant to define a compact ID space for the IDed entity, but you want to define where the ID space starts, i.e. the minimum ID. Adamant allows a developer to specify minimum on a per ID type basis using the following pattern within an assembly model: \\
...
Data dependency ID specification is done in a slightly different manner, via the \texttt{set\_data\_dependencies} tag within the assembly. For example, within the \textit{science\_assembly.assembly.yaml} model, Section \ref{Modeling an Assembly}, we map the \texttt{science\_Instance}'s two data dependencies to two external data products in the system, i.e. \texttt{"sensor\_1\_Reading => data\_Collector\_Instance.Sensor\_1\_Data"} which maps the \texttt{science\_Instance}'s \texttt{sensor\_1\_Reading} data dependency to the \texttt{data\_Collector\_Instance}'s \texttt{Sensor\_1\_Data} data product. Adamant will ensure that both the data product and data dependency exist in the assembly and are of the same type. If these checks pass, then the IDs of the data dependencies will be resolved to the appropriate data product IDs in the \texttt{Set\_Id\_Bases} procedure within the assembly package's autocoded body. In this way, components can safely share data among one another while retaining their compile-time independent property.
...
\vspace{5mm} %5mm vertical space
\begin{spaceditemize}
  \item \textbf{\texttt{component\_name}} - Filter by component instance names, i.e. \texttt{Command\_Router\_Instance}.
  \item \textbf{\texttt{component\_type}} - Filter by component type names, i.e. \texttt{Example\_Command\_Router}.
...
\begin{spaceditemize}
  ...
  \item \textbf{\texttt{connector\_type}} - Filter by connector type i.e. something like \texttt{Command.T}.
  \item \textbf{\texttt{connector\_kind}} - Filter by connector kind, i.e. \texttt{recv\_async}, \texttt{recv\_sync}, \texttt{send}, \texttt{request}, \texttt{service}, \texttt{get}, \texttt{return}, \texttt{provide}, or \texttt{modify}
\end{spaceditemize}
\vspace{5mm} %5mm vertical space
...
\vspace{5mm} %5mm vertical space
\begin{spaceditemize}
  \item \textbf{\texttt{data\_dependency\_name}} - Filter by data dependency or data product names, i.e. \texttt{Register\_Stuffer\_Consumer\_Instance.Last\_Register\_Written} or \texttt{Register\_Stuffer\_Instance.Last\_Register\_Written}.
  \item \textbf{\texttt{data\_dependency\_type}} - Filter by type passed along a data dependency, i.e. \texttt{Register\_Value.T}.
\end{spaceditemize}
\vspace{5mm} %5mm vertical space
...
\yamlcodef{../example_architecture/data_collector_view.science_assembly.view.yaml}
...
\begin{figure}[H]
  \includegraphics[width=1.0\textwidth,center]{../example_architecture/build/eps/science_assembly_data_collector_view.eps}
\end{figure}
...
To include a C or C++ library we must first include the library in the Adamant build path, just like we would with Ada source code, i.e.
...
Alternatively, the analysis mode can be controlled using the \texttt{REDO\_ANALYZE\_MODE} environment variable. If set, this variable will pass the \texttt{---mode=<VALUE>} parameter to the \texttt{gnatsas analyze} command. For example:
...
This approach allows users to easily switch analysis modes without changing build targets. If \texttt{REDO\_ANALYZE\_MODE} is not set, no mode parameter is passed and GNAT SAS runs in its default \texttt{fast} mode. \\

For automated testing and continuous integration, the \texttt{redo analyze\_all} command provides additional control over exit behavior through the \texttt{REDO\_ANALYZE\_ALL\_FAIL\_LEVEL} environment variable. This variable determines the analysis warning level that causes the command to exit with a non-zero code:

\vspace{5mm} %5mm vertical space
\begin{spaceditemize}
  \item \textbf{\texttt{HIGH}} - Exit with failure only on high-severity warnings or errors (default)
  \item \textbf{\texttt{MEDIUM}} - Exit with failure on medium, high-severity warnings, or errors  
  \item \textbf{\texttt{LOW}} - Exit with failure on low, medium, high-severity warnings, or errors
  \item \textbf{\texttt{ERROR}} - Exit with failure only on analysis errors (not warnings)
\end{spaceditemize}
\vspace{5mm} %5mm vertical space
...
To create the generated Ada package we can run \texttt{build/obj/Linux/system_registers.o} which produces the output.

\vspace{5mm} %5mm vertical space
\inputminted{text}{../example_architecture/register_map/output.txt}
\vspace{5mm} %5mm vertical space

This is not the normal output we expect to see when generating an Ada package. Enforcement for bit-constrained safety is done with deterministic compile-time checks in the generated Ada itself. Specifically, each map item is guarded with \texttt{Compile\_Time\_Error (not <Type>.Always\_Valid, ...)} so unsafe types are rejected during normal compilation. \\

A bit-constrained type is a type that for some bit value can become invalid, causing a \texttt{Constraint\_Error} to be thrown at runtime. While bit-constrained types are useful for catching bugs in most Ada code, they can be troublesome when interfacing with volatile hardware interfaces, where the compiler is not in control of what might be written to these types. In such cases, a hardware interface may write an invalid bit value to a type within a register map. When this type is later read by the Ada program, a \texttt{Constraint\_Error} will be thrown. Here we trigger on a \texttt{Packed\_Natural} type which contains a Natural, whose range is $0$ to $2^{31} - 1$, stored inside a 32-bit value. Due to this, we can not safely read a Natural from the register map without fear of reading something out of range that could trigger a \texttt{Constraint_Error}, i.e. \texttt{0xFFFF_FFFF}. To prevent this, Adamant does not allow such types in register maps. If a bit-constrained type is used, compilation fails with a clear compile-time error and the specification is rejected. \\
...
To create the generated Ada package we can run \texttt{redo build/obj/Linux/nonvolatile_store.o} which produces the output.
...
A bit-constrained type is a type that for some bit value can become invalid, causing a \texttt{Constraint\_Error} to be thrown at runtime. While bit-constrained types are useful for catching bugs in most Ada code, they can be troublesome when interfacing with volatile hardware interfaces, where the compiler is not in control of what might be written to these types. In such cases, a hardware interface may write an invalid bit value to a type within a memory map. When this type is later read by the Ada program, a \texttt{Constraint\_Error} will be thrown. Here we trigger on a \texttt{Packed\_Natural} type which contains a Natural, whose range is $0$ to $2^{31} - 1$, stored inside a 32-bit value. Due to this, we can not safely read a Natural from the memory map without fear of reading something out of range that could trigger a \texttt{Constraint_Error}, i.e. \texttt{0xFFFF_FFFF}. To prevent this, Adamant does not allow such types in memory maps. If a bit-constrained type is used, compilation fails with a clear compile-time error and the specification is rejected. \\
...
\vspace{5mm} %5mm vertical space
\begin{spacedenumerate}
  \item Adding environment variables to the shell's configuration file (i.e. \textit{.bashrc}).
...
which sets the \textit{build target} to the test version for the current target, i.e. if the \texttt{TARGET} is currently \texttt{Linux} then the \textit{env.py} will change \texttt{TARGET} for the test directory to \texttt{Linux\_Test} instead, unless the current \texttt{TARGET} ends in \texttt{Coverage}. This \textit{env.py} shows the standard Adamant unit test environment and should be included in every unit test directory. \\
