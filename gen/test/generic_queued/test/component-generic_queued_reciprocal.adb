package body Component.Generic_Queued_Reciprocal is
   ---------------------------------------
   ...
   begin
      ...
      case Ret is
         ...
         when Message_Dropped =>
            ...
            Generic_Type_2_Send_Dropped (Base_Instance'Class (Self), Arg);
      end case;
   end Generic_Type_2_Send;

   not overriding procedure Attach_Aa_T_Send (Self : in out Base_Instance; To_Component : in not null Component.Class_Access; Hook : in not null Aa_T_Send_Connector.Invokee_Hook; To_Index : in Connector_Index_Type := Connector_Index_Type'First) is
   begin
      Self.Connector_Aa_T_Send.Attach (To_Component, Hook, To_Index);
   end Attach_Aa_T_Send;

   not overriding procedure Aa_T_Send (Self : in out Base_Instance; Arg : in Aa.T; Full_Queue_Behavior : in Full_Queue_Action := Drop) is
      Ret : Connector_Status;
   begin
      Ret := Aa_T_Send_Connector.Call (Self.Connector_Aa_T_Send, Arg, Full_Queue_Behavior);
      case Ret is
         -- Send worked fine, just continue on.
         when Success => null;
         when Message_Dropped =>
